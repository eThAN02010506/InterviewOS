...
function renderLive() {
  ...
  if ($('live-review')) {
    ...
    $('live-review').innerHTML = pendingCandidateSegments.length ? `<div class="review-summary"><strong>${pendingCandidateSegments.length} 条待确认</strong><span>${confirmedCount} 条已归档 · ${esc(readiness)}</span></div>${pendingCandidateSegments.map(segment => `<div class="review-claim"><div><small>候选人回答 #${esc(segment.sequence)}</small><span>${esc(segment.text.slice(0, 180))}${segment.text.length > 180 ? '…' : ''}</span></div><div class="claim-actions"><button type="button" data-live-edit="${esc(segment.id)}">修改</button><button type="button" data-live-confirm="${esc(segment.id)}">确认证据</button></div></div>`).join('')}` : `暂无待确认候选人回答；已归档 ${confirmedCount} 条证据。${esc(readiness)}`;
  }
