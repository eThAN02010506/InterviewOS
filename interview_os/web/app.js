...
window.addEventListener('pagehide',()=>{if(liveRecorder?.state==='recording')liveRecorder.stop();if(sessionRecordingActive||sessionChunks.length){stopSessionRecorder();api.raw(`/api/live-interviews/${state.sessionId}/audio/final`,{method:'POST',keepalive:true,body:(()=>{const fd=new FormData();const b=new Blob(sessionChunks,{type:'audio/webm'});sessionChunks=[];fd.append('file',b,`session-${state.sessionId.slice(0,8)}.webm`);return fd;})()}).catch(()=>{});}});
...
$('session-audio-download')?.addEventListener('click',async()=>{if(!state.sessionId)return;try{const blob=await api.blob(`/api/live-interviews/${state.sessionId}/audio`);const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`interview-${state.sessionId.slice(0,8)}.wav`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);toast('全场录音已下载');}catch(error){toast(error.message,true)}});
...
async function streamNextSuggestion() {
  ...
  try {
    const resp = await api.raw(`/api/live-interviews/${state.sessionId}/suggestions/stream`, {method:'POST', signal: liveSuggestionAbort.signal});
