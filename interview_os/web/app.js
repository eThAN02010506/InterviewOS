...
async function loadMockAnswerAudio(responseId){
...
$('live-stop-record').onclick=()=>{if(liveRecorder?.state==='recording'){asrPreviewSequence+=1;asrPreviewBusy=false;liveRecorder.stop();$('live-stop-record').disabled=true;$('live-recording-note').textContent='录制结束，正在确认最终转写…'}};
...
function startContinuousRecorder(preferred){if(!liveMediaStream)return;liveRecorder=new MediaRecorder(liveMediaStream,preferred?{mimeType:preferred}:undefined);liveRecorder.ondataavailable=event=>{if(!event.data.size)return;if(!liveVadUtteranceChunks.length)liveVadSpeechStart=performance.now();liveVadUtteranceChunks.push(event.data);liveVadUtteranceBytes+=event.data.size;previewRecordedChunks(liveVadUtteranceChunks,liveRecorder?.mimeType||preferred,showLiveAsrPreview);if(liveVadUtteranceBytes>20*1024*1024&&liveRecorder?.state==='recording'){liveVadState='finalizing';finalizeLiveUtterance();}};liveRecorder.onstop=stopLiveContinuousVad;liveRecorder.start(VAD_TIMESLICE_MS);}
function analyzeLiveVadLevel(){if(!liveVadAnalyser||!liveVadData||!liveContinuousMode)return;if(liveVadAudioContext?.state==='suspended'){liveVadAudioContext.resume();return;}liveVadAnalyser.getFloatTimeDomainData(liveVadData);let sum=0;for(let i=0;i<liveVadData.length;i++)sum+=liveVadData[i]*liveVadData[i];const rms=Math.sqrt(sum/liveVadData.length);const now=performance.now();const speaking=rms>=VAD_SPEECH_RMS;if(liveVadState==='idle'){if(now-liveVadLastFinalizeAt<VAD_GRACE_MS)return;if(speaking){resetAsrPreview();liveVadState='in-speech';liveVadSpeechStart=now;liveVadLastSpeech=now;}}else if(liveVadState==='in-speech'){if(speaking){liveVadLastSpeech=now;}else if(now-liveVadLastSpeech>=VAD_SILENCE_MS){if(now-liveVadSpeechStart>=VAD_MIN_UTTERANCE_MS){liveVadState='finalizing';finalizeLiveUtterance();}else{liveVadState='idle';liveVadSpeechStart=0;liveVadLastSpeech=0;}}}updateLiveVadNote();}
