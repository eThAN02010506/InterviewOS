...
function renderMock() {
  ...
  const questionLabel=displayedAsFollowUp?'证据追问':current?.source==='custom'?`自定义问题${current.competency&&current.competency!=='自定义问题'?` · ${current.competency}`:''}`:(current?.competency||'综合能力');
...
$('custom-question-form').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget;if(!await ensureSession())return;const question=$('custom-question-text').value.trim();if(!question)return;busy(form,true);try{await api(`/api/mock-interviews/${state.sessionId}/questions`,{method:'POST',body:JSON.stringify({question,competency:$('custom-question-competency').value.trim(),practice_now:true})});$('custom-question-text').value='';$('custom-question-competency').value='';$('custom-question-panel').open=false;mockRetry=false;mockRetryResponseId='';await loadSession();toast('自定义问题已加入，开始练习');}catch(error){toast(error.message,true)}finally{busy(form,false)}};
