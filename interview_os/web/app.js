...
function renderLive() {
  ...
  const confirmedCount = (state.session?.live_interview_records || []).filter(record => record.source === 'live_interview').length;
  ...
  $('live-transcript').innerHTML = segments.length ? segments.map(segment => {
    ...
    const evidenceAction = isCandidate ? (recorded ? '<small class="evidence-confirmed">已归档为证据</small>' : `<button class="btn compact" data-live-confirm="${esc(segment.id)}">确认为证据</button>`) : '';
  ...
  if ($('live-review')) {
    ...
    $('live-review').className = pendingCandidateSegments.length ? 'review-queue' : 'review-queue empty-state';
    $('live-review').innerHTML = pendingCandidateSegments.length ? `<div class="review-summary"><strong>${pendingCandidateSegments.length} 条待确认</strong><span>${confirmedCount} 条已归档</span></div>${pendingCandidateSegments.map(segment => `<div class="review-claim"><div><small>候选人回答 #${esc(segment.sequence)}</small><span>${esc(segment.text.slice(0, 180))}${segment.text.length > 180 ? '…' : ''}</span></div><div class="claim-actions"><button type="button" data-live-edit="${esc(segment.id)}">修改</button><button type="button" data-live-confirm="${esc(segment.id)}">确认证据</button></div></div>`).join('')}` : `暂无待确认候选人回答；已归档 ${confirmedCount} 条证据。`;
  }
...
$('live-confirm-all').onclick=async()=>{const defaultCompetency=state.session?.job?.competencies?.[0]||'';const competency=window.prompt('批量确认使用的能力维度；留空则由系统按问题推断',defaultCompetency)||'';const button=$('live-confirm-all');busy(button,true);try{const data=await api(`/api/live-interviews/${state.sessionId}/evidence/batch`,{method:'POST',body:JSON.stringify({competency})});state.session=data.state;renderAll();toast('待确认候选人回答已批量归档');}catch(error){toast(error.message,true)}finally{busy(button,false)}};
