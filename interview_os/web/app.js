...
$('mock-voice').onclick=async()=>{if(!navigator.mediaDevices?.getUserMedia||!window.MediaRecorder){toast('当前浏览器不支持麦克风',true);return;}try{mockVoiceStream=await navigator.mediaDevices.getUserMedia({audio:true});const preferred=liveAudioType();mockVoiceRecorder=new MediaRecorder(mockVoiceStream,preferred?{mimeType:preferred}:undefined);mockVoiceChunks=[];mockVoiceRecorder.ondataavailable=e=>{if(e.data.size)mockVoiceChunks.push(e.data)};mockVoiceRecorder.onstop=()=>{mockVoiceRecorder=null;};mockVoiceRecorder.start(250);$('mock-voice').disabled=true;$('mock-stop-voice').disabled=false;mockVoiceNote('正在录音，说完后点“停止并转写”');}catch(error){if(mockVoiceStream){mockVoiceStream.getTracks().forEach(t=>t.stop());mockVoiceStream=null;}toast(`无法使用麦克风：${error.message}`,true);}};
$('mock-stop-voice').onclick=async()=>{const recorder=mockVoiceRecorder;const stream=mockVoiceStream;mockVoiceRecorder=null;mockVoiceStream=null;if(!recorder){mockVoiceNote('');return;}$('mock-stop-voice').disabled=true;mockVoiceNote('正在转写…');recorder.onstop=async()=>{const type=recorder.mimeType||'audio/webm';const blob=new Blob(mockVoiceChunks,{type});mockVoiceChunks=[];stream?.getTracks().forEach(t=>t.stop());try{const wav=await encodeBlobAsWav(blob);const form=new FormData();form.append('file',wav,'answer.wav');const data=await api(`/api/mock-interviews/${state.sessionId}/transcribe`,{method:'POST',body:form});$('mock-answer').value=(data.text||'').trim();$('mock-voice').disabled=false;mockVoiceNote('已转写，可编辑后提交');}catch(error){$('mock-voice').disabled=false;mockVoiceNote('');toast(`语音转写失败：${error.message}`,true);}};recorder.stop();};
